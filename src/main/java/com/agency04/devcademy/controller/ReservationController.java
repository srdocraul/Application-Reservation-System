...
@RestController @RequestMapping("/reservations") @Slf4j public class ReservationController {

    @Autowired private ReservationService reservationService;

    @GetMapping public ResponseEntity<List<Reservation>> getAllReservation() {
        log.debug("Get all reservations");
        return ResponseEntity.ok().body(reservationService.getAllReservation());
    }

    @GetMapping("/{id}") public ResponseEntity<Object> getReservationById(@PathVariable(value = "id") String id) {
    ...
    @PutMapping("/{id}") public ResponseEntity<ReservationForm> updateReservation(@PathVariable Long id,
                                                                                  @RequestBody ReservationForm reservationForm) {
        reservationForm.setId(id);
        return ResponseEntity.ok().body(this.reservationService.createReservationCommand(reservationForm));
    }

    @DeleteMapping("/{id}") public HttpStatus deleteReservation(Long id) {
